...
def get_dc_cable_lengths(seed_str="", kwp=0.0, arr_counts=(0, 0, 0, 0)):
    """Menghasilkan anggaran panjang kabel DC (m) yang berbeza & realistik bagi setiap pelanggan."""
    h = sum(ord(c) * 11 for c in seed_str) if seed_str else 42
    extra_len = 15 if kwp >= 14.0 else (10 if kwp >= 10.0 else 0)

    l1 = 65 + (h % 35) + extra_len       # 65m - 115m
    l2 = 50 + ((h * 3) % 35) + extra_len # 50m - 95m
    l3 = 40 + ((h * 7) % 30) + extra_len # 40m - 80m
    l4 = 50 + ((h * 13) % 30) + extra_len # 50m - 90m

    # Bulatkan ke gandaan 5m terdekat
    l1 = int(round(l1 / 5.0) * 5)
    l2 = int(round(l2 / 5.0) * 5)
    l3 = int(round(l3 / 5.0) * 5)
    l4 = int(round(l4 / 5.0) * 5)

    return (
        l1 if arr_counts[0] > 0 else 0,
        l2 if arr_counts[1] > 0 else 0,
        l3 if arr_counts[2] > 0 else 0,
        l4 if arr_counts[3] > 0 else 0
    )
...
def get_mapped_values_for_template(project_data):
    ...
    l1, l2, l3, l4 = get_dc_cable_lengths(client_key, kwp_val, (arr_1, arr_2, arr_3, arr_4))
